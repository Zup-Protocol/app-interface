...
""",
  ...
  test(
    "When calling 'updateNativeTokenAndFetch' and the signer is null, it should not get the user token balance",
    () async {
      final sut0 = TokenAmountCardUserBalanceCubit(
        wallet,
        tokenAddress,
  ...
  test(
    "When calling `updateNativeTokenAndFetch` and the signer is not null, it should get the user token balance",
    () async {
      final sut0 = TokenAmountCardUserBalanceCubit(
        wallet,
        tokenAddress,
  ...
  test(
    """When calling `updateNativeTokenAndFetch`
  it should update the network variable for the one passed,
  so the next time to fetch the balance it will use the new network""",
    () async {
      ...
      verify(
        () => wallet.nativeOrTokenBalance(any(), rpcUrl: nextNetwork.rpcUrl),
      ).called(2); // two times because of the initial load
    },
  );

  test(
    """When calling `updateTokenAndNetwork` passsing the is native true, it should update the isNative variable,
  and when a new signer is emitted it will get the native balance""",
    () async {
      final signerStreamController = StreamController<Signer?>.broadcast();
